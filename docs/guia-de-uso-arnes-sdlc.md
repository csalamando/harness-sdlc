...
- **CODEOWNERS (frontera dura)**: `codeowners_gen.py` (v2.33.4) deriva `.github/CODEOWNERS` desde la matriz + el roster diligenciado — una línea por artefacto con los titulares reales del rol dueño, roles sin persona comentados como SIN TITULAR, y `--check` anti-drift en CI (tocar matriz o roster sin regenerar = fallo). Con branch protection ("Require review from Code Owners"), un PR que toca `spec/adr/` no se mergea sin revisión del Arquitecto. (La plantilla manual `CODEOWNERS-template` quedó retirada: derivaba en drift.)
