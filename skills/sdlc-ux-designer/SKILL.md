...
1. **Estructura gobernada en `spec/ux/`** (owner `ux-designer` en la matriz de autoridad):
   ...
   - **Excepción documentada (v2.33.1)**: cuando las pantallas prototipadas pertenecen al *design system del propio arnés* (no a un proyecto gobernado), el artefacto puede vivir en `docs/design-system/ux/` junto a los tokens canónicos que alimenta — es el caso del portal del arnés (PANT-01..08). La excepción aplica solo al repo del arnés; todo proyecto gobernado usa `spec/ux/`. En ambos casos el gobierno es idéntico: inventario + archivo versionado + exports + recibo.
...
4. **Cambio = re-aprobación**: si cambian las HU, los flujos UX, los roles **o los tokens del design system** (`tokens.json` es upstream del inventario desde v2.33.2 — un cambio de tokens invalida el prototipo aprobado), `spec_diff_impact.py` revoca el recibo de `spec/ux/` y las pantallas impactadas deben actualizarse y re-aprobarse. El prototipo nunca se edita "en silencio": todo cambio visible para el usuario pasa por revisión humana.
